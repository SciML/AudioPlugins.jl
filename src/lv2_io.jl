...
Waveform code for `lv2_in_tone`: a square wave alternating between `+amplitude`
and `-amplitude`.
"""
...
Waveform code for `lv2_in_tone`: a sawtooth rising from `-amplitude` to
...
Waveform code for `lv2_in_tone`: a single non-zero sample per period, zero
elsewhere.
"""
...
function lv2_test_bundle(; force::Bool = false)
    ...
    root = @get_scratch!("test_plugins_lv2-$(Sys.ARCH)")
    ...
    if force || stale(bin)
        ...
        mkpath(bundle)
        run(`$cc $(_c_arch_flags()) -O2 -fPIC -shared -Wall -Wextra -o $bin $src`)
...
Name the open plugin gives itself in its manifest, or an empty string when
nothing is open.
"""
...
Block size the open plugin was instantiated with, in frames. Fixed at
[`lv2_open!`](@ref) and unchanged until the next open.
"""
...
Number of host audio channels the open plugin was connected for; the `channels`
[`lv2_open!`](@ref) was called with.
"""
...
How many `run()` calls the host has made since the last
[`lv2_reset_counters!`](@ref). One block in must be exactly one call, so this is
what proves the host is not re-running or coalescing blocks.
"""
...
Number of control input ports the open plugin exposes; the length of
[`lv2_params`](@ref).
"""
...
is authoritative after the first block. **Not compensated**: the opt-in
Julia-side compensation [`clap_open!`](@ref) offers has no LV2 counterpart,
...
Zero the host's call counters, so a following [`lv2_n_process`](@ref) counts
...
"""
    AudioPlugins.lv2_in_tone(t, waveform, freq, amp) -> token

The LV2 counterpart of [`AudioPlugins.clp_in_tone`](@ref): same arguments, same
return, over the open LV2 plugin. `waveform` is an `LV2_WAVE_*` code.
"""
...
"""
    AudioPlugins.lv2_process(dep, id0, v0, id1, v1, id2, v2, id3, v3) -> token
...
"""
    AudioPlugins.lv2_out_rms(dep) -> Float64

The LV2 counterpart of [`AudioPlugins.clp_out_rms`](@ref).
"""
lv2_out_rms(dep) = ccall((:lv2_out_rms, LV2_LIB), Cdouble, (Cdouble,), dep)
"""
    AudioPlugins.lv2_out_peak(dep) -> Float64

The LV2 counterpart of [`AudioPlugins.clp_out_peak`](@ref).
"""
lv2_out_peak(dep) = ccall((:lv2_out_peak, LV2_LIB), Cdouble, (Cdouble,), dep)
"""
    AudioPlugins.lv2_out_valid(dep) -> Float64

The LV2 counterpart of [`AudioPlugins.clp_out_valid`](@ref).
"""
...
The LV2 counterpart of [`AudioPlugins.clp_out_sample`](@ref).
"""
