...
@testset "AudioPlugins / VST3" begin
    ...
    @testset "the host comes prebuilt, the test plugins do not" begin
        ...
        @test isdir(VST3_BUNDLE)
        @test !startswith(VST3_BUNDLE, pkgdir(AudioPlugins))
    end

    @testset "discovery" begin
        classes = vst3_scan(VST3_BUNDLE)
    ...
    @testset "failure paths are loud" begin
        ...
        @test_throws ErrorException vst3_open!(VST3_BUNDLE; class_id = "0"^32, block_size = 64)
        @test occursin("no audio-effect class", vst3_last_error())
        @test_throws ErrorException vst3_open!(VST3_BUNDLE; class_id = GAIN, block_size = 99999)
        @test_throws ErrorException vst3_open!(VST3_BUNDLE; class_id = GAIN, block_size = 64, channels = 7)
        @test_throws ErrorException vst3_open!(VST3_BUNDLE; class_id = GAIN, sample_rate = -1, block_size = 64)
        @test !vst3_is_open()
    end

    @testset "open reports the configuration actually in force" begin
        vst3_open!(VST3_BUNDLE; class_id = GAIN, sample_rate = 48000, block_size = 64)
        @test vst3_is_open()
    ...
    @testset "stereo keeps its channels apart" begin
        vst3_open!(VST3_BUNDLE; class_id = GAIN, block_size = 64, channels = 2)
    ...
    @testset "latency is reported (and documented as uncompensated)" begin
        vst3_open!(VST3_BUNDLE; class_id = GAIN, block_size = 64)
        @test vst3_latency() == 0.0
        vst3_open!(VST3_BUNDLE; class_id = LOOK, block_size = 64)
    ...
    @testset "state persists across blocks (the invariant that matters)" begin
        a = 0.25
        vst3_open!(VST3_BUNDLE; class_id = POLE, block_size = 32)
        split = Float64[]
        for _ in 1:2
            t = vst3_fill!(ones(32))
        ...
        vst3_open!(VST3_BUNDLE; class_id = POLE, block_size = 64)
        t = vst3_fill!(ones(64))
        o = AP.vst3_process(t, 0, a, -1, 0, -1, 0, -1, 0)
        whole = vst3_out(o)
    ...
    @testset "reopening is a fresh instance" begin
        ...
        @test vst3_out(o)[1] ≈ 0.25 atol = 1.0e-6
    end
