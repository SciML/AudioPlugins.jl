...
Waveform code for `vst3_in_tone`: a square wave alternating between `+amplitude`
and `-amplitude`.
"""
...
Waveform code for `vst3_in_tone`: a sawtooth rising from `-amplitude` to
...
Waveform code for `vst3_in_tone`: a single non-zero sample per period, zero
elsewhere.
"""
...
function vst3_scan(path::AbstractString)
    ...
    return [
        (
            id = unsafe_string(ccall((:vst3_host_scan_id, VST3_LIB), Cstring, (Clong,), i)),
...
"""
function vst3_open!(
        path::AbstractString; class_id::AbstractString = "",
        sample_rate::Real = 48000, block_size::Integer = 512,
        channels::Integer = 1
    )
    r = ccall(
        (:vst3_host_open, VST3_LIB), Cint,
...
"""
    vst3_plugin_name() -> String

Name the open plugin's class reports for itself, or an empty string when
nothing is open.
"""
vst3_plugin_name() = unsafe_string(ccall((:vst3_host_plugin_name, VST3_LIB), Cstring, ()))

"""
    vst3_plugin_id() -> String

Class id of the open plugin as 32 hex characters — its identity in VST3 — or an
...
Whether a plugin is currently open. A failed [`vst3_open!`](@ref) leaves this
...
Block size the open plugin was set up with, in frames. Fixed at
[`vst3_open!`](@ref) and unchanged until the next open.
"""
vst3_block_size() = Int(ccall((:vst3_host_block_size, VST3_LIB), Cdouble, ()))

"""
    vst3_sample_rate() -> Float64
...
Number of host audio channels the plugin's main buses were arranged for; the
`channels` [`vst3_open!`](@ref) was called with.
"""
...
How many `IAudioProcessor::process()` calls the host has made since the last
[`vst3_reset_counters!`](@ref). One block in must be exactly one call, so this
is what proves the host is not re-processing or coalescing blocks.
"""
vst3_n_process() = ccall((:vst3_host_n_process, VST3_LIB), Clong, ())

"""
    vst3_param_count() -> Int

Number of parameters the open plugin's edit controller exposes; the length of
[`vst3_params`](@ref).
"""
...
compensated**: the opt-in Julia-side compensation [`clap_open!`](@ref) offers
has no VST3 counterpart, so a caller that cares must align the stream itself.
"""
...
Zero the host's call counters, so a following [`vst3_n_process`](@ref) counts
...
function vst3_params()
    ...
    return [
        (
            id = ccall((:vst3_host_param_id, VST3_LIB), Cdouble, (Clong,), i),
...
function vst3_fill!(samples::AbstractVector{<:Real}; channels::Integer = 1)
    ...
    return ccall(
        (:vst3_in_fill, VST3_LIB), Cdouble, (Ptr{Cdouble}, Clong, Clong),
...
function vst3_out(token::Real; channel::Integer = 0)
    ...
    return [
        ccall(
            (:vst3_out_sample, VST3_LIB), Cdouble, (Cdouble, Cdouble, Cdouble),
            token, i, channel
        ) for i in 0:(Int(n) - 1)
    ]
end
...
"""
    AudioPlugins.vst3_in_tone(t, waveform, freq, amp) -> token

The VST3 counterpart of [`AudioPlugins.clp_in_tone`](@ref): same arguments, same
return, over the open VST3 plugin. `waveform` is a `VST3_WAVE_*` code.
"""
vst3_in_tone(t, waveform, freq, amp) =
    ccall(
    (:vst3_in_tone, VST3_LIB), Cdouble, (Cdouble, Cdouble, Cdouble, Cdouble),
    t, waveform, freq, amp
)

"""
    AudioPlugins.vst3_process(dep, id0, v0, id1, v1, id2, v2, id3, v3) -> token

The VST3 counterpart of [`AudioPlugins.clp_process`](@ref). The `id`s are VST3
...
"""
    AudioPlugins.vst3_out_rms(dep) -> Float64

The VST3 counterpart of [`AudioPlugins.clp_out_rms`](@ref).
"""
vst3_out_rms(dep) = ccall((:vst3_out_rms, VST3_LIB), Cdouble, (Cdouble,), dep)
"""
    AudioPlugins.vst3_out_peak(dep) -> Float64

The VST3 counterpart of [`AudioPlugins.clp_out_peak`](@ref).
"""
vst3_out_peak(dep) = ccall((:vst3_out_peak, VST3_LIB), Cdouble, (Cdouble,), dep)
"""
    AudioPlugins.vst3_out_valid(dep) -> Float64

The VST3 counterpart of [`AudioPlugins.clp_out_valid`](@ref).
"""
vst3_out_valid(dep) = ccall((:vst3_out_valid, VST3_LIB), Cdouble, (Cdouble,), dep)
"""
    AudioPlugins.vst3_in_sample(dep, i, ch) -> Float64

The VST3 counterpart of [`AudioPlugins.clp_in_sample`](@ref).
"""
vst3_in_sample(dep, i, ch) =
    ccall((:vst3_in_sample, VST3_LIB), Cdouble, (Cdouble, Cdouble, Cdouble), dep, i, ch)
"""
    AudioPlugins.vst3_out_sample(dep, i, ch) -> Float64

The VST3 counterpart of [`AudioPlugins.clp_out_sample`](@ref).
"""
